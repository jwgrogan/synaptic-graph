...
fn test_explicit_save_redacts_secrets() {
    ...
    assert!(result.is_ok(), "explicit_save should succeed even with secrets");
    let impulse = result.unwrap();
    assert!(
        !impulse.content.contains("sk-abc123def456ghi789jkl012mno345pqr678"),
...
fn test_explicit_save_with_connections() {
    ...
    assert!(result.is_ok(), "explicit_save_with_connections should succeed");
    let impulse = result.unwrap();
    assert_eq!(impulse.status, ImpulseStatus::Candidate);
...
fn test_save_empty_content_fails() {
    ...
    assert!(result_whitespace.is_err(), "whitespace-only content should fail");
}
